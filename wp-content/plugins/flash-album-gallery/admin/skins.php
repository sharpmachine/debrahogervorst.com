...
<?php } ?>
		<li><a href="#" rel="skinoptions"><?php _e('Active Skin Options', 'flash-album-gallery'); ?></a></li>
	</ul>

<?php if( current_user_can('FlAG Add skins') ) { ?>
	<div id="addskin" class="cptab">
		<h2><?php _e('Add new skin', 'flash-album-gallery'); ?></h2>
		<div>&nbsp;</div>
		...
		<form method="post" enctype="multipart/form-data" action="<?php echo admin_url('admin.php?page=flag-skins'); ?>">
			...
			<input type="submit" class="button" name="installskin" value="<?php _e('Install Now', 'flash-album-gallery'); ?>" /></p>
		</form>
...
<?php } ?>
	...
	<div id="skinoptions" class="cptab">
		<h2><?php _e('Active Skin Options', 'flash-album-gallery'); ?></h2>
...
<?php if( current_user_can('FlAG Add skins') ) { ?>
<div id="skinapikey">
	<h2><?php _e('Skin License Key', 'flash-album-gallery'); ?></h2>
	...
	<form method="post" enctype="multipart/form-data" action="<?php echo admin_url('admin.php?page=flag-skins'); ?>">
		...
		<input type="submit" class="button" value="<?php _e('Save', 'flash-album-gallery'); ?>" /></p>
	</form>
</div>
<?php } ?>
...
<p style="float: right; display:none;"><a class="button" href="<?php echo wp_nonce_url('admin.php?page=flag-skins&amp;skins_refresh=1', 'skins_refresh'); ?>"><?php _e('Refresh / Update Skins', 'flash-album-gallery'); ?></a></p>
<p><a class="button<?php if(!$type) echo '-primary'; ?>" href="<?php echo admin_url('admin.php?page=flag-skins'); ?>"><span style="font-size: 14px;"><?php _e('Photo skins', 'flash-album-gallery'); ?></span></a>&nbsp;&nbsp;&nbsp;
<a class="button<?php if($type == 'm') echo '-primary'; ?>" href="<?php echo admin_url('admin.php?page=flag-skins&amp;type=m'); ?>"><span style="font-size: 14px;"><?php _e('Music skins', 'flash-album-gallery'); ?></span></a>&nbsp;&nbsp;&nbsp;
<a class="button<?php if($type == 'v') echo '-primary'; ?>" href="<?php echo admin_url('admin.php?page=flag-skins&amp;type=v'); ?>"><span style="font-size: 14px;"><?php _e('Video skins', 'flash-album-gallery'); ?></span></a>&nbsp;&nbsp;&nbsp;
<a class="button<?php if($type == 'b') echo '-primary'; ?>" href="<?php echo admin_url('admin.php?page=flag-skins&amp;type=b'); ?>"><span style="font-size: 14px;"><?php _e('Banner skins', 'flash-album-gallery'); ?></span></a>&nbsp;&nbsp;&nbsp;
<a class="button<?php if($type == 'w') echo '-primary'; ?>" href="<?php echo admin_url('admin.php?page=flag-skins&amp;type=w'); ?>"><span style="font-size: 14px;"><?php _e('Widget skins', 'flash-album-gallery'); ?></span></a>
</p>
...
function flag_curl_exec_follow($ch, &$maxredirect = null) {
  ...
  return curl_exec($ch);
	...
	if(!empty($skins_xml)) {
		...
		$skins_xml_error = __('cURL library is not installed on your server.','flash-album-gallery').'<br>'.__('Download skins from http://codeasily.com/portfolio/grand-flagallery-skins/', 'flash-album-gallery');
	}
...
<?php
	...
	<tr class="<?php echo $class; ?> second">
		...
		if(is_dir($settings)) { ?>
			<a class="thickbox" href="<?php echo FLAG_URLPATH.'admin/skin_options.php?show_options=1&amp;skin='.dirname($skin_file).'&amp;TB_iframe=1&amp;width=600&amp;height=560'; ?>"><?php _e('Options', 'flash-album-gallery' ); ?></a>
 		<?php }
		if ( current_user_can('FlAG Delete skins') ) {
		if ( dirname($skin_file) != $flag_options['flashSkin'] ) { ?>
			<br /><br /><a class="delete" onclick="javascript:return flag_delskin('<?php echo $skin_data['Name']; ?>');" href="<?php echo wp_nonce_url('admin.php?page=flag-skins&delete='.dirname($skin_file), 'delete_skin'); ?>" title="<?php _e( 'Delete this skin', 'flash-album-gallery' ); ?>"><?php _e('Delete', 'flash-album-gallery' ); ?></a>
		<?php }
 		} ?>
		</td>
	</tr>
