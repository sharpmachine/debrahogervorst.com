...
<?php if ($comments) : ?>
	<h3 id="comments">This article has <?php comments_number('no comments', 'one comment', '% comments' );?> so far!</h3>
	<ol class="commentlist">
		<?php foreach ($comments as $comment) : ?>
		<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
			<cite><?php comment_author_link() ?></cite> says &mdash;
			<?php if ($comment->comment_approved == '0') : ?>
			<em>(your comment is awaiting moderation)</em>
			<?php endif; ?>
			<br />
			<small class="commentmetadata">(On <a href="#comment-<?php comment_ID() ?>" title=""><?php comment_date('F jS, Y') ?> at <?php comment_time() ?></a><?php edit_comment_link('e &hellip;',' | ',''); ?>)</small>
			<?php comment_text() ?>
		</li>
...
<h3 id="respond">Leave a Comment</h3>
	...
	<label for="url">Website / Blog</label></p>
	<?php endif; ?>
